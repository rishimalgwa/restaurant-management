...
func CreateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		validatotionErr := validate.Struct(invoice)
		if validatotionErr != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": validatotionErr.Error()})
			return
		}
...
func UpdateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprintf("invoice item updation failed")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
		defer cancle()
